export async function fetchSummoner(name, tag, count = 20, start = 0) {
  const url = `/api/summoner?name=${encodeURIComponent(name)}&tag=${encodeURIComponent(tag)}&count=${count}&start=${start}`
